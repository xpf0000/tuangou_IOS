...
class ShopSearchVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,UISearchDisplayDelegate
{
    ...
    override func viewDidLoad() {
        ...
        httphandle.modelClass=TuanModel.self
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        DataCache.Share.ShopSearchKey.add(str: searchBar.text!)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if searchTxt == ""
        {
            return DataCache.Share.ShopSearchKey.keyArr.count+1
        }
        else
        {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if searchTxt == ""
        {
            ...
            if row < DataCache.Share.ShopSearchKey.keyArr.count
            {
                cell.textLabel?.textAlignment = .left
                cell.textLabel?.text = DataCache.Share.ShopSearchKey.keyArr[row]
            }
            else
            {
        ...
        else
        {
            ...
            cell.model = httphandle.listArr[row] as! TuanModel
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if searchTxt == ""
        {
            if row == DataCache.Share.ShopSearchKey.keyArr.count
            {
                DataCache.Share.ShopSearchKey.clean()
                table.reloadData()
            }
            else
            {
                searchbar.text = DataCache.Share.ShopSearchKey.keyArr[row]
